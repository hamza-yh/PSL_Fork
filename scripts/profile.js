    ...
    return `
        ...
        <div class="set-solve ${ (matchIndex === 0 && si === 0) ? '' : 'collapsed' }">    
            ${set.solves.map((solve, i) => `
                            <div class="align-right solve-time ${solve.win ?"":"dark"}">${displayTime(solve.competitorTime) ?? "N/A"}</div>
                            ...
                            <div class="align-left solve-time ${solve.win ?"dark":""}">${displayTime(solve.opponentTime) ?? "N/A"}</div>
                        `).join("")}
                    </div>
                </div>
            `).join("")}
...
async function renderProfile() {
    ...
    <div>
        ...
        <div class="left-stats">
            <div>
                <div class="value solve-time">${displayTime(data.bestSingle ?? -2)}</div>
                <div class="label">BEST SINGLE</div>
            </div>
            <div >
                <div class="value solve-time">${displayTime(data.seasonMean ?? -2)}</div>
                <div class="label">SEASON MEAN</div>
            </div>
            <div>
                <div class="value solve-time">${displayTime(data.elimMean ?? -2)}</div>
